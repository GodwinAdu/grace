...
const SingleBlog: React.FC<SermonProps> = ({ blog }) => {
  const { _id, title, imageUrl, sermonTopic, author, tags, publishDate } = blog;
  return (
    <>
      <div
        ...
        data-wow-delay=".1s"
      >
        <Link href={`/sermon/${_id}`} className="relative block h-[220px] w-full">
          ...
          <Image className="w-full h-full" width={100} height={100} src={imageUrl} alt="image" />
        </Link>
        <div className="p-6 sm:p-8 md:py-8 md:px-6 lg:p-8 xl:py-8 xl:px-5 2xl:p-8">
          ...
          <div className="flex items-center">
            <div className="mr-5 flex items-center border-r border-body-color border-opacity-10 pr-5 dark:border-white dark:border-opacity-10 xl:mr-3 xl:pr-3 2xl:mr-5 2xl:pr-5">
              <div className="mr-4">
                <div className="relative h-10 w-10 overflow-hidden rounded-full">
                  <Image unoptimized fill src={author.photo} alt="author" />
                </div>
              </div>
              <div className="w-full">
                ...
                <p className="text-xs text-body-color">{author.duty}</p>
              </div>
            </div>
            <div className="inline-block">
              ...
              <p className="text-xs text-body-color">{moment(publishDate).format("MMMM Do YYYY")}</p>
            </div>
          </div>
        </div>
      </div>
